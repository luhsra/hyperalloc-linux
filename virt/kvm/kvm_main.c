...
__kvm_handle_hva_range(struct kvm *kvm, const struct kvm_hva_range *range)
{
...
kvm_handle_hva_range_no_flush(struct mmu_notifier *mn, unsigned long start,
			      unsigned long end, hva_handler_t handler)
{
	struct kvm *kvm = mmu_notifier_to_kvm(mn);
	const struct kvm_hva_range range = {
		.start = start,
					    ...
					    struct kvm_map_region *map_region)
{
	...
	if (!atomic_read(&vcpu->kvm->online_vcpus))
		return -EINVAL;
	...
	if (added && map_region->nr_pages > 0)
		ret = -EAGAIN;
					  ...
					  struct kvm_map_region *map_region)
{
  ...
  pages = (struct page **) kzalloc(map_region->nr_pages * sizeof(struct page *), GFP_KERNEL);
  ...
  pinned_pages = get_user_pages_fast(map_region->source_addr, map_region->nr_pages, 0, pages);

  if (pinned_pages < 0) {
    printk(KERN_WARNING "kvm_vm_ioctl_map_memory_region: pinning failed completely");
    goto out;
  }

  if (pinned_pages != map_region->nr_pages) {
    printk(KERN_WARNING "kvm_vm_ioctl_map_memory_region: could only pin %d out of %lu\n", pinned_pages, map_region->nr_pages);
    goto out;
  }
...
out_free1:
	...
	case KVM_GET_SREGS: {
		kvm_sregs =
			kzalloc(sizeof(struct kvm_sregs), GFP_KERNEL_ACCOUNT);
		r = -ENOMEM;
		if (!kvm_sregs)
			goto out;
			    ...
			    const char *fmt)
{
	int ret;
	struct kvm_stat_data *stat_data =
		(struct kvm_stat_data *)inode->i_private;
