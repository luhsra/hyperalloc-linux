...
void noinline llfree_copy_into_buffer(llfree_zone_info_t *llfree_info, void *buffer) {
	if (!buffer) {
	...
	*qemu_llfree = *llfree_info->qemu_llfree;
	...
	llfree_info_buffer->zone_llfree_huge_page_counter = llfree_info->zone_llfree_huge_page_counter;
	...
	llfree_info_buffer->zone_free_pages = (_Atomic(int64_t) *) virt_to_phys(llfree_info_buffer->zone_free_pages);
	llfree_info_buffer->zone_llfree_huge_page_counter = (_Atomic(int64_t) *) virt_to_phys(llfree_info_buffer->zone_llfree_huge_page_counter);
